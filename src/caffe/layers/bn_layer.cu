...
namespace caffe {
	...
	void BNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		...
		if (this->phase_ == TRAIN) {
		    caffe_gpu_axpby(batch_statistic_.count(), (num_)*decay_/(num_-1), batch_statistic_.gpu_data(), Dtype(1) - (num_)*decay_/(num_-1),
			    this->blobs_[3]->mutable_gpu_data());
		    // DEBUG
		    //Dtype* tmp_statistic = batch_statistic_.mutable_cpu_data();
		    //LOG(INFO) << "---------------" << decay_;
		    //for (int i = 0; i < batch_statistic_.count(); ++i){
		    //	LOG(INFO) << "variance " << i << ": " << *(tmp_statistic+i);
		    //}
		}
		if (this->phase_ == TEST && moving_average_) {
